...
HCScreen::HCScreen(Adafruit_ST7735& tft): _tft(tft) {
  ...
  _fontColor = convertColor(HC_WHITE);
  _bgColor = convertColor(HC_DARKBG);
  _selFontColor = convertColor(HC_DARKBG);
  _selBgColor = convertColor(HC_WHITE);
  _titleFontColor = convertColor(HC_CYAN);
  _titleBgColor = convertColor(HC_BLACK);
  _gridSelColor = convertColor(HC_RED);
  _gridBgColor = convertColor(HC_DARKBG);
  _title = "";
  _startLine = 0;
  _contentLines = 0;
  _showTitle = 0;
